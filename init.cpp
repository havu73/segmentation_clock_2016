...
void calc_max_delay_size (input_params& ip, sim_data& sd, rates& rs, double** sets) {
	...
	if (sd.big_gran > sd.max_delay_size) { //warning if the big_gran is greater than the maximum delay time
...
/* create_mutant_data fills in the name, knockouts, conditions, etc. for each mutant
	...
	Ha 20160517:What does single_mutant mean?
*/
mutant_data* create_mutant_data (sim_data& sd, input_params& ip) {
	...
	if (sd.num_active_mutants == 2) {
		if (MUTANT_DELTA != 1) {// 20160519: Right now it is equal to 1, so single_mutant is always false
			single_mutant = true;
		}
	}
	...
	for (int i = 0; i < sd.num_active_mutants; i++) {
		mds[i].index = i;
		//sd.active_start right now is initialized to be sd.width_initial - 1
